...
| Active sessions metric / list / kill from UI   | `GET /My/database-user-cache?onlyInUse=true` returning `{userName, databaseServerId, databaseId, since}`, plus a new api-key-authenticated kill endpoint: the existing `KillDatabaseUser` takes the end user's SuperTokens token (`[Authorize]`), so the portal cannot call it. | `database_user_cache.in_use` already tracks this; only a list endpoint is missing. |
...
5. The container's `/api/*` proxy calls the gateway by its public hostname, and
   the tasks get no public IP, so give them a way out: a NAT gateway, or
   private DNS for the gateway plus a route to it. They also need to reach ECR
   to pull the image (the NAT gateway, or ECR VPC endpoints). See
   `deploy/README.md`.
